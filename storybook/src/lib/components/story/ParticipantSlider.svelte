...
import {
		...
		sliderScrollResetSignal
	} from '$lib/stores/participantStore.js';
import ParticipantSliderTutorial from './ParticipantSliderTutorial.svelte';
...
const tutorialSteps = [
	{
		...
		description:
			'Clique nos blocos dos gráficos acima para trazer primeiro quem respondeu daquela forma. Clique novamente no mesmo bloco para voltar ao estado original.'
	},
	{
		title: 'Foque em uma pessoa',
		description:
			'Selecione um participante na barra para ver todas as respostas dele em destaque. Clique novamente para remover o foco.'
	},
	{
		title: 'Abra a ficha completa',
		description:
			'Use o botão “+” ao lado da foto para abrir o modal com a ficha completa dessa pessoa.'
	}
];
let tutorialStep = 1;
...
function toggleMobileFilters() {
	...
	function getRosterSignature(list = []) {
		if (!Array.isArray(list) || !list.length) return 'empty';
		return list.map((participant) => participant?.id || '').join('|');
	}
	...
	$: {
		const scrollResetValue = $sliderScrollResetSignal ?? 0;
		if (scrollResetValue !== lastScrollResetValue) {
			lastScrollResetValue = scrollResetValue;
			pendingScrollReset = true;
		}
	}
	function resetScrollPosition() {
		...
		try {
			...
			/* scrollTo not supported – scrollLeft already set */
		}
	...
	$: if (scroller && pendingScrollReset) {
...
{#if allParticipants.length}
	<div
		...
		class:slider-shell--tutorial-active={tutorialVisible}
		class:slider-shell--tutorial-step-1={tutorialVisible && tutorialStep === 1}
		class:slider-shell--tutorial-step-2={tutorialVisible && tutorialStep === 2}
		class:slider-shell--tutorial-step-3={tutorialVisible && tutorialStep === 3}
		style={shellStyle}
	>
		{#if tutorialVisible}
			<ParticipantSliderTutorial
				step={tutorialStep}
...
<style>
	...
	.slider-shell--tutorial-step-1 .stories-slider {
		outline: 2px solid rgba(249, 115, 22, 0.75);
		border-radius: 18px;
		box-shadow: 0 0 35px rgba(249, 115, 22, 0.35);
	}

	.slider-shell--tutorial-step-2 .story-item {
		opacity: 0.25;
	}

	.slider-shell--tutorial-step-2 .story-item:first-child {
		opacity: 1;
		transform: translateY(-4px);
		box-shadow: 0 10px 18px rgba(0, 0, 0, 0.35);
	}

	.slider-shell--tutorial-step-3 .story-more {
