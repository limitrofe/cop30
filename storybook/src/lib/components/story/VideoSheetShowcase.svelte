<script>
	...
	import {
		...
		buildSections,
		buildFeedSourceVideos,
		buildShortzVideos,
		shuffleList,
		moveFirstDifferent,
		createSeededRandom
	} from '$lib/utils/videoSheetProcessing.js';
	import {
		...
		defaultVideoConfig,
		defaultLayoutConfig
	} from '$lib/utils/videoSheetConfig.js';
	...
	let precomputedVideos = hydratePrecomputedVideos(initialData?.precomputed?.videos);
	let precomputedFilterOptions = Array.isArray(initialData?.precomputed?.filterOptions)
		? initialData.precomputed.filterOptions
		: null;
	let chunkConfig = null;
	const chunkLoaded = new Set();
	const chunkLoaders = new Map();
	let chunkLoadAllPromise = null;
	let chunkPrefetchTimeoutId = null;
	const initialChunkIndex = initialData ? resolveChunkIndexFromUrl(sheetUrl) || 1 : null;
	if (initialChunkIndex) {
		chunkLoaded.add(initialChunkIndex);
	}
	...
	let feedPrimaryVideoId = null;
	let lastIsMobileFeed = false;
	let feedRenderLimit = MOBILE_FEED_INITIAL_CHUNK;
	let feedRenderedVideos = [];
	let lastFeedVideosRef = null;
	let feedExtendSentinel;
	let feedExtendObserver = null;
	const MOBILE_FEED_INITIAL_CHUNK = 6;
	const MOBILE_FEED_CHUNK_INCREMENT = 6;
	const MOBILE_FEED_CHUNK_THRESHOLD = 2;
	...
	$: chunkConfig = normalizeChunkConfig(chunkedSheet);
	$: {
		if (chunkConfig && browser && chunkLoaded.size < chunkConfig.total) {
			scheduleChunkPrefetch(600);
		} else {
			cancelChunkPrefetch();
		}
	}
	...
	$: {
		if (precomputedVideos?.length) {
			videos = precomputedVideos;
	...
	$: filterOptions =
		precomputedFilterOptions ?? buildFilterOptions(videos, resolvedFilterColumns, filtersResolved);
	$: {
	...
	$: {
		if (isMobileFeed && feedVideos.length) {
			...
			feedPrimaryVideoId = feedVideos[0]?.uuid ?? null;
	...
	$: if (lastFeedVideosRef !== feedVideos) {
		lastFeedVideosRef = feedVideos;
		resetFeedRenderLimit();
	}
	$: if (!isMobileFeed) {
		if (feedRenderLimit !== feedVideos.length) {
			feedRenderLimit = feedVideos.length;
		}
	} else if (feedRenderLimit > feedVideos.length) {
		feedRenderLimit = feedVideos.length;
	}
	$: if (isMobileFeed && feedVideos.length) {
		const activeIndex = Math.max(0, getFeedIndex(activeFeedId));
		const remaining = feedRenderLimit - activeIndex;
		if (remaining <= MOBILE_FEED_CHUNK_THRESHOLD) {
			expandFeedRenderLimit();
		}
	}
	$: feedRenderedVideos =
		!isMobileFeed || !feedVideos.length
			? feedVideos
			: feedVideos.slice(0, Math.min(feedRenderLimit, feedVideos.length));
	$: {
	...
	$: if (isMobileFeed !== lastIsMobileFeed) {
	...
	$: {
		if (
			isMobileFeed &&
			feedRenderedVideos.length < feedVideos.length &&
			feedExtendSentinel &&
			mobileFeedContainer
		) {
			setupFeedExtendObserver();
		} else {
			teardownFeedExtendObserver();
		}
	}
	...
	async function loadSheet() {
		...
		try {
			...
			if (chunkConfig) {
				const fetchedIndex = resolveChunkIndexFromUrl(sheetUrl) || 1;
				chunkLoaded.add(fetchedIndex);
				scheduleChunkPrefetch(600);
			}
		} catch (err) {
	...
	function hydratePrecomputedVideos(list) {
		if (!Array.isArray(list)) return null;
		return list.map((video) => {
			const source = video || {};
			const filterIds =
				source.filterIds instanceof Set
					? source.filterIds
					: new Set(Array.isArray(source.filterIds) ? source.filterIds : []);
			return {
				...source,
				filterIds
			};
	...
	function normalizeChunkConfig(config) {
		if (!config || typeof config !== 'object') return null;
		const pattern = typeof config.pattern === 'string' ? config.pattern : '';
		const total = Number(config.chunkTotal) || 0;
		if (!pattern || total <= 1) return null;
		const size = Number(config.chunkSize) || null;
		return {
			pattern,
			total,
			size
	...
	function formatChunkUrl(pattern, index) {
		...
		return pattern.replace(/\{\{\s*index(?::0?(\d+))?[a-z]*\s*\}\}/gi, (_, pad) => {
			const padSize = pad ? Number(pad) : 0;
			const value = String(index);
			return padSize ? value.padStart(padSize, '0') : value;
		});
	}

	function resolveChunkIndexFromUrl(url) {
		if (!url || typeof url !== 'string') return null;
		const match = url.match(/(\d+)(?=[^0-9]*$)/);
		return match ? Number(match[1]) : null;
	}

	async function loadChunk(index, reason = 'auto') {
		if (!browser || !chunkConfig) return;
		if (index < 1 || index > chunkConfig.total) return;
		if (chunkLoaded.has(index)) return;
		if (chunkLoaders.has(index)) return chunkLoaders.get(index);
		const url = formatChunkUrl(chunkConfig.pattern, index);
		if (!url) return;
		const loader = (async () => {
			try {
				const response = await (fetcher ?? fetch)(url, { headers: fetchHeaders });
				if (!response.ok) {
				...
				if (payload?.meta) {
					sheetMeta = payload.meta;
				}
				const chunkPrecomputed = payload?.precomputed;
				const hydratedVideos = hydratePrecomputedVideos(chunkPrecomputed?.videos);
				if (hydratedVideos?.length) {
					precomputedVideos = precomputedVideos
						? [...precomputedVideos, ...hydratedVideos]
						: hydratedVideos;
				} else if (rows.length && !precomputedVideos) {
					sheetRows = [...sheetRows];
				}
				if (!precomputedFilterOptions && Array.isArray(chunkPrecomputed?.filterOptions)) {
					precomputedFilterOptions = chunkPrecomputed.filterOptions;
				}
				chunkLoaded.add(index);
				if (chunkConfig && chunkLoaded.size < chunkConfig.total) {
					scheduleChunkPrefetch(1200);
				}
			} catch (error) {
				console.warn(`VideoSheetShowcase: falha ao carregar chunk ${index} (${reason})`, error);
			} finally {
				chunkLoaders.delete(index);
			}
		})();
		chunkLoaders.set(index, loader);
		return loader;
	}

	async function loadNextChunk(reason = 'auto') {
		if (!chunkConfig) return;
		for (let index = 1; index <= chunkConfig.total; index += 1) {
			if (!chunkLoaded.has(index)) {
				await loadChunk(index, reason);
				break;
			}
		}
	}

	function triggerFullChunkLoad(reason = 'user') {
		if (!chunkConfig) return;
		if (chunkLoadAllPromise) return chunkLoadAllPromise;
		chunkLoadAllPromise = (async () => {
			for (let index = 1; index <= chunkConfig.total; index += 1) {
				await loadChunk(index, reason);
			}
		})()
			.catch((error) => {
				console.warn('VideoSheetShowcase: falha ao carregar todos os chunks', error);
			})
			.finally(() => {
				chunkLoadAllPromise = null;
			});
		return chunkLoadAllPromise;
	}

	function scheduleChunkPrefetch(delay = 800) {
		if (!browser || !chunkConfig) return;
		if (chunkPrefetchTimeoutId) return;
		if (chunkLoaded.size >= chunkConfig.total) return;
		chunkPrefetchTimeoutId = setTimeout(
			() => {
				chunkPrefetchTimeoutId = null;
				loadNextChunk('prefetch');
			},
			Math.max(0, delay)
		);
	}

	function cancelChunkPrefetch() {
		if (chunkPrefetchTimeoutId) {
			clearTimeout(chunkPrefetchTimeoutId);
			chunkPrefetchTimeoutId = null;
		}
	}
	...
	// data helpers moved to $lib/utils/videoSheetProcessing.js
	...
	function handleSearchInput(event) {
		...
		if (chunkConfig && searchTerm?.trim()) {
			triggerFullChunkLoad('search');
		}
		if (searchResolved.instant) {
			activeSearchTerm = searchTerm;
	...
	function shouldRenderFeedPlayer(videoId) {
		if (!isMobileFeed || !videoId) return false;
		if (!feedPlayerWindow.size) {
			const fallback = activeFeedId ?? feedVideos[0]?.uuid ?? null;
			return videoId === fallback;
		}
		return feedPlayerWindow.has(videoId);
	}

	function shouldSkipFeedAds(video) {
		if (feedAdsDisabled) return true;
		if (!video) return false;
		if (video.uuid && feedPrimaryVideoId && video.uuid === feedPrimaryVideoId) {
			return true;
		}
		return resolveSkipDFPFlag(video);
	}
	...
	function getFeedIndex(videoId) {
		if (!videoId) return -1;
		const index = feedIndexLookup.get(videoId);
		return typeof index === 'number' ? index : -1;
	}

	function expandFeedRenderLimit(amount = MOBILE_FEED_CHUNK_INCREMENT) {
		if (!isMobileFeed || !feedVideos.length) return;
		const cappedAmount = Math.max(1, amount);
		const next = Math.min(feedRenderLimit + cappedAmount, feedVideos.length);
		if (next !== feedRenderLimit) {
			feedRenderLimit = next;
		}
		if (chunkConfig) {
			loadNextChunk('feed').catch(() => {});
		}
	}

	function resetFeedRenderLimit() {
		if (!isMobileFeed) {
			feedRenderLimit = feedVideos.length;
		} else {
			const baseLimit = Math.min(Math.max(MOBILE_FEED_INITIAL_CHUNK, 1), feedVideos.length || 0);
			feedRenderLimit = baseLimit;
		}
	}

	function setupFeedExtendObserver() {
		if (!browser) return;
		if (!feedExtendSentinel) return;
		teardownFeedExtendObserver();
		try {
			feedExtendObserver = new IntersectionObserver(
				(entries) => {
					for (const entry of entries) {
				...
				{
					root: mobileFeedContainer ?? null,
					rootMargin: '40% 0px',
					threshold: 0.1
				}
			);
			feedExtendObserver.observe(feedExtendSentinel);
		} catch (error) {
			console.warn('VideoSheetShowcase: falha ao observar sentinel de feed', error);
		}
	}

	function teardownFeedExtendObserver() {
		if (!feedExtendObserver) return;
		try {
			feedExtendObserver.disconnect();
		} catch (error) {
			console.warn('VideoSheetShowcase: falha ao encerrar sentinel de feed', error);
		}
		feedExtendObserver = null;
	}
	...
	{:else if isMobileFeed}
		{#if feedRenderedVideos.length}
			<div class="mobile-feed-shell">
				...
				>
					{#each feedRenderedVideos as video, index (video.uuid)}
						{@const videoIndex = getFeedIndex(video.uuid)}
						<article
							role="listitem"
						...
						>
							<div
								...
								style={video.thumbnail && shouldRenderFeedPlayer(video.uuid)
									? `--mobile-feed-thumb:url(${JSON.stringify(video.thumbnail)})`
									: undefined}
							>
								{#if video.thumbnail && shouldRenderFeedPlayer(video.uuid)}
									<img
										src={video.thumbnail}
										alt={video.title}
										loading="lazy"
								...
								{#if shouldRenderFeedPlayer(video.uuid)}
									<GloboPlayer
										...
										startMuted={isMobileFeed
											? shouldAutoMute && !feedAudioUnlocked
											: shouldAutoMute}
										controls={true}
										aspectRatio="9 / 16"
										...
										skipDFP={shouldSkipFeedAds(video)}
									...
									/>
								{:else}
									<div class="mobile-feed__player-ghost" aria-hidden="true"></div>
								{/if}
							</div>
								...
								<div class="mobile-feed__content">
									...
									>
										{#if showCountsEnabled}
											<span class="mobile-feed__counter">
												{(videoIndex >= 0 ? videoIndex : index) + 1}/{feedVideos.length}
											</span>
										{/if}
					...
					{/each}
					<div
						class="mobile-feed__extend-sentinel"
						bind:this={feedExtendSentinel}
						aria-hidden="true"
					></div>
				</div>
			</div>
