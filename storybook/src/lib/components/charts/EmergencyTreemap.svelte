<script>
	...
	function render() {
		...
		const nodes = svg.selectAll('g.cell').data(leaves, (d) => d.data.name);
		...
		updateLabelOverlay(leaves);
		updateSelectionHighlight();
	}

	function ensureLabelLayer() {
		if (labelLayer || !container) {
			return labelLayer;
		}
		labelLayer = document.createElement('div');
		labelLayer.className = 'treemap-label-layer';
		container.appendChild(labelLayer);
		return labelLayer;
	}

	function updateLabelOverlay(leaves = []) {
		const layer = ensureLabelLayer();
		if (!layer) return;
		layer.style.width = `${width}px`;
		layer.style.height = `${height}px`;

			const labelSelection = select(layer)
		...
		const labelEnter = labelSelection
			...
			.html(
				'<span class="treemap-label__title"></span><span class="treemap-label__value"></span>'
			);
		...
		const labelMerged = labelEnter.merge(labelSelection);
			...
			labelMerged
				...
				.style('color', '#2c1e19')
			.attr('data-category', (d) => d.data.name)
			.classed('treemap-label--compact', (d) => Math.min(d.x1 - d.x0, d.y1 - d.y0) < 140)
			.each(function (d) {
				const node = select(this);
				node.select('.treemap-label__title').text(d.data.name);
				node.select('.treemap-label__value').text(formatter(d.value || 0));
			});
	...
	function updateSelectionHighlight() {
		...
		const hasSelection = Boolean(highlightGroup);
		svg.classed('has-selection', hasSelection);
...
<style>
	...
	:global(.treemap-label-layer .treemap-label) {
		position: absolute;
	...
	:global(.treemap-label-layer.has-selection .treemap-label) {
		opacity: 0.35;
	}

	:global(.treemap-label-layer.has-selection .treemap-label.active) {
		opacity: 1;
	}

	:global(.treemap-label__title) {
		font-size: clamp(0.92rem, 1.6vw, 1.28rem);
		font-weight: 400;
	}

	:global(.treemap-label__value) {
		font-size: clamp(1.05rem, 1.9vw, 1.6rem);
		font-weight: 600;
	}

	:global(.treemap-label--compact .treemap-label__title) {
		font-size: clamp(0.78rem, 1.4vw, 1.05rem);
	}

	:global(.treemap-label--compact .treemap-label__value) {
